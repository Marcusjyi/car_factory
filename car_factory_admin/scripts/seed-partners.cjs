/**
 * Seed partners collection.
 *
 *   cd car_factory_admin
 *   npm run seed-partners
 *
 * Credentials: same as create-admin (.env.local / service-account.json)
 */
...
function initAdmin() {
  ...
  if (!sa) {
    console.error("Firebase Admin credential missing. See create-admin.cjs.");
    process.exit(1);
  }
  initializeApp({
    credential: cert(sa),
...
async function main() {
  ...
  console.log(`Seeding partners into database "${DATABASE_ID}"...`);
