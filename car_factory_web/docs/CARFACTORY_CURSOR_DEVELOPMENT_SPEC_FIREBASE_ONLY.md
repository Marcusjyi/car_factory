...
export interface ProductDocument {
  id: string; // products/{id} 문서 ID (users.uid 대응)
  sellerUid: string; // 판매자 Auth UID
  title: string;
  description: string;
  categoryId: string;
  partName: string; // 필수
  ...
  partNumber: string; // 필수 — 향후 부품번호 DB
  ...
  shippingFee: number;
  region: string | null;
  // 사진은 Storage, 여기엔 URL만
  images: Array<{
    path: string; // Storage path
    downloadURL: string; // getDownloadURL
    width: number;
    height: number;
    sortOrder: number;
  }>;
  thumbnailURL: string; // images[0].downloadURL
...
추가 배송지 목록이 필요할 때 사용하는 서브컬렉션.
기본 주소·기본 배송지는 `users/{uid}.address` / `users/{uid}.shippingAddress`에 저장한다.
...
- 이미지 1개 최대 10MB
- 상품당 최대 10개
- **원본은 Cloud Storage에만 저장** (`product-images/{sellerUid}/{productId}/{imageId}.jpg`)
- Firestore `images[]`에는 `path` + `downloadURL`(Storage URL)만 저장
- `thumbnailURL` = 첫 이미지 `downloadURL`
